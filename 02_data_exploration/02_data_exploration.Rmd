...
```{r}
rm(qog_excel, qog_stata, qog_spss, qog_text)
```

To remove everything from your environment (all objects and custom functions), you can use the `rm(list=ls())`. Note: this will only delete the objects in your environment, but the packages stay loaded!
...
```{r echo=FALSE, fig.cap="source: VanderPlas, Jake. Python data science handbook: Essential tools for working with data. " O'Reilly Media, Inc.", 2016 (ch3)}
