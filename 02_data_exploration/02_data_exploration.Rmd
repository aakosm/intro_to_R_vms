...
After you installed a given package we need to load it to be able to use its functions. We do this with the `library()` command. It is good practice that you load all the packages at the beginning of your script.
...
qog_text <- readr::read_delim("data/qog_bas_cs_jan18.csv", delim = ",")
```
...
```{r}
qog_excel <- readxl::read_excel("data/qog_bas_cs_jan18.xlsx")
```
...
qog_stata <- haven::read_stata("data/qog_bas_cs_jan18.dta")

# read the SPSS .sav file
qog_spss <- haven::read_spss("data/qog_bas_cs_jan18.sav")

```


To remove the unnecessary objects from the memory, we can use the `rm()` function.
```{r}
rm(qog_excel, qog_stata, qog_spss)
```
...
```{r, eval=FALSE}
saveRDS(qog_text, file = "data/output/qog_rda.Rda")
```
...
We will use the `gapminder` package that contains macroeconomic data from the famous Hans Rosling presentation on life expectancy and regional convergence.
...
1.**group our data by continent** and then 
2. **create a summary variable**, that tells us the mean GDP for each continent. 
...
Using data visualization is a great way to get acquainted with your data and sometimes it makes more sense than looking at large tables. In this section we get into the `ggplot2` package which we'll use throughout the class. It is the cutting edge of R's data visualization toolset (not just in academia, but in business and data journalism as well).
...
ggplot(data = gapminder_df,
  ...
  labs(x = "GDP per capita", # add captions and axis titles
       y = "Life expectancy",
