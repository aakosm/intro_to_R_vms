...
> **Contact info**
>
> -   Akos: [Mate.Akos\@tk.mta.hu](mailto:Mate.Akos@tk.hu){.email} <br>
> -   Eszter: [farkas_eszter\@phd.ceu.edu](mailto:farkas_eszter@phd.ceu.edu){.email} <br>
> -   Daniel: [Kovarek_Daniel\@phd.ceu.edu](mailto:Kovarek_Daniel@phd.ceu.edu){.email}

> **Course info and materials**
>
> -   <https://ceulearning.ceu.edu/course/view.php?id=9930>
...
*There are some sections marked (OPTIONAL), these ones I won't cover due to time limit, but you can check them out on your own and have a go at them.*
...
```{r out.width = "600px", echo=FALSE, fig.cap=""}
...
*Source: [https://stackoverflow.blog/2017/10/10/impressive-growth-r/](Stack%20Overflow%20blog)*
...
-   De-mystify coding, hopefully be a positive first impression
...
-   Cover all the steps of the research workflow: data cleaning -\> data wrangling -\> exploratory analysis -\> modeling -\> reporting
-   Preparing you on how to get into R more if you want to

## Course outline

> -   Monday 22 ꟷ Friday 26 March 2021
> -   \~2.5 hours of live teaching per day (this includes breaks, Q&A)

The course is organized into four blocks, each a logical unit. The actual topics that we are going to cover fit into 8 categories, so we have 2 sessions' length of time buffer (previous years' experience shows that the 8 sessions actually fill the allocated time).
...
R comes with a pretty spartan GUI so we will work with the RStudio IDE (integrated development environment). In the first part of the course we will use **RStudio Cloud**, which a browser based option and let's us get into R very quickly. Late in the course you'll install RStudio on your computer and learn to set up projects and everything necessary to carry out your own research.
...
Using Projects with RStudio will simplify your and your collaborators' life. Essentially, **all your project related files are collected in your selected folder so you don't need to specify a working directory.** Your project will be able to run as long as you copy the entire folder.
...
> -   R is case sensitive, so use lowercase file and variable names. Separate words with underscore `_` (e.g.: `ols_1`) (or you can do the camelCase thing, but be consistent)
...
A great one-stop-shop resource for online R books:

> [Big Book of R](https://www.bigbookofr.com/)

**Since R is an open source project it is a common courtesy to cite R and the packages you use**, as people (often working in academia) put many hours into developing tools and it is in our common interest to give some public recognition to these efforts and contributions. To see how to cite R or you can just type the following:
...
**NOTE:** This first part is tedious. I get it. I know. Unfortunately these knowledge building block are necessary to cover before we get to the juicy parts so we speak the same language (pun sort of intended). Please bear with me during this session I know for some it is boring because you know this, for some it seems to make no sense (it will, I promise) and for some this seems overly technical (I tried to keep the bare minimum). With this caveat, let's start coding in R!
...
If you are in doubt what are the arguments of the given function, you can use `?help(function_name)` or `?function_name`.
...
```{r, echo=FALSE, fig.cap="Wickham, Hadley: Advanced R, O'Reilly (ch3)"}
knitr::include_graphics("figures/atomic_vector.png")
```
...
list2 <- list(
    letters = letters[1:6], 
    numbers = c(1:3), 
    logical = c(TRUE, TRUE, FALSE, FALSE, TRUE)
    )
...
You can change the type of data inside a vector. This is fairly straightforward and not used regularly. The most common use cases include the conversion of strings into some other data format (to numbers or dates) From strings to numbers:
...
Coercing functions starts with `as.*`, where \* marks the datatype. Start typing `as.` in RStudio and see how many functions are suggested with this beginning.

NB: "When you attempt to combine different types they will be coerced in a fixed order: character → double → integer → logical." (from [Hadley Wickham's Advanced R](https://adv-r.hadley.nz/vectors-chap.html#testing-and-coercion))
