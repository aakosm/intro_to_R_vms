...
>
> - [Suzan Baert's blogpost series](https://suzanbaert.netlify.app/2018/01/dplyr-tutorial-1/) (a bit old but still great for starting out)
...
This session will focus on various ways to subset our data using the tools of the `dplyr` package. We will use the gapminder data for demonstrations and for the missing data section the Mammals sleep dataset.
...
If you are just interested in the top results, you can select rows by their position with the `dplyr::slice()` function.

```{r}
slice(gapminder_df, 1:8) # select the first 8 rows
```

Some examples of certain logical operators. To see the role of each line, check the comments in the code snippet below.
```{r collapse=FALSE}
gapminder_df %>% 
    filter(continent == "Africa" & gdpPercap > 8000) # AND

gapminder_df %>% 
    filter(!continent %in% c("Africa", "Europe") ) # everything but Africa and Europe (note the position of the ! operator)
...
You can have remove columns with `select(data, -column)`. This code removes columns between year and GDP per capita. 
```{r}
gapminder_df %>% 
...
`dplyr` makes it easy to recode our columns and create new ones with the `dplyr::mutate()` function. `mutate()` let's you do all the stuff that we covered when we looked at vectors. You have the option to have the calculation results in a new column (preferable) or overwrite an existing one (probably not the best idea).
...
Finally, there are two very useful functions for recoding variables. `if_else()` from `dplyr` (the base R function which is basically the same is `ifelse()`). The logic of the function is the same as the one found in Excel: `if_else(condition, value_if_true, value_if_false)`. As a quick exercise, we are going to create a new variable and use `if_else` to see if a country is below or above the average GDP per capita from the sample.

```{r}
gapminder_df %>% 
    select(country, year, gdpPercap) %>% 
    mutate(gdp_position = if_else(gdpPercap > mean(gdpPercap), "above", "below"))
```

If we would need more categories (say, higher income, middle income, lower income) one way would be nesting these conditional functions but nested ifelse gets ugly really fast. Rather, we should use the `case_when` function which is a way better solution for this. The `case_when` can assign values based on multiple conditions relating to a given variable. To get a more visual result, let's restrict the year to 1992 with the `filter()` function.

To make an informed decision, let's see the distribution for the GDP per capita.
...
gapminder_df %>% 
    ...
    summary()
```
Now we can set up our new `country_type` variable using `case_when`. The syntax of the function is the conditional value comes after the condition and a `~` and the various conditions are then separated with a comma.
...
gapminder_df %>% 
    ...
    select(country, year, gdpPercap) %>% 
    mutate(
        country_type = case_when(gdpPercap < 1270 ~ "lower_income",
                                 gdpPercap > 1270 & gdpPercap < 8100 ~ "middle_income",
                                 gdpPercap > 8100 ~ "higher_income" )
        )
...
gapminder_df %>% 
    ...
    select(id, everything()) # everything() is a nice helper function if you want to move one important column to the beginning
```


Important to know with `left_join` and `right_join`: if on the left/right side of join more than one observation is linked to the unique id on the main dataset, you'll get duplicated rows. More on this discussion: https://stackoverflow.com/questions/49256920/left-join-in-r-dplyr-too-many-observations


# Dealing with missing data

For this, we'll use a data set with sleep data on mammals (from the ggplot2 package) which is called `msleep`. Notice the `NA`s in the summary statistics.
